...
class MapSuite extends FunSuite with Matchers{
  test("Apply on an empty Map should shield None"){
  ...
  test("add on an empty Map should yield a new Map with two element"){
    val first = randomString
    val second = randomString
  ...
  test("remove on an empty Map should yield an empty Map"){
    ...
    val strillEmpty = Map.empty.remove(element)
    strillEmpty(element) shouldBe None
  }

  test("remove on a non empty Map should yield a new Map without element"){
  ...
  test("add/remove combo should be ensure that all elements are distinct"){
    val key = randomString

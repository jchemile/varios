...
class Aplicacion:
    ...
    def guardar(self):
        ...
        if nombrearch != '':
            archil = open(nombrearch, "w", encoding="utf-8")
            archil.write(self.scrolledtext1.get("1.0",tk.END))
            archil.close()
            mb.showinfo("informacion", "Losdatos fueron guardados en el archivo")

    def recuperar(self):
        nombrearch = fd.askopenfilename(initialdir= "/", title="Seleccione archiv", filetypes=(("txt files", "*.txt"), ("todos los archivos", "*.*")))
        if nombrearch != '':
            archil = open(nombrearch, "r", encoding="utf-8")

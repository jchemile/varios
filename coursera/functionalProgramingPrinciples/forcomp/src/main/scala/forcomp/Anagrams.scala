...
object Anagrams {
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def iter(ocurrences: Occurrences): List[Sentence] = {
      if (ocurrences.isEmpty) List(Nil)
      else for{
        combination <- combinations(ocurrences)
        word <- dictionaryByOccurrences getOrElse (combination, Nil)
        sentence <- iter(subtract(ocurrences, wordOccurrences(word)))

...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val newAcc: TweetSet = elem match {
      case x if p(x) => acc.incl(x)
      case x => acc
    }
  ...
  def mostRetweetedAcc(acc: Tweet): Tweet = {
    if(acc.retweets < elem.retweets) remove(elem).mostRetweetedAcc(elem)
    else remove(elem).mostRetweetedAcc(acc)
  }

  def descendingByRetweetAcc(acc: TweetList): TweetList = {
    val r = mostRetweeted
    new Cons(r, remove(r).descendingByRetweetAcc(acc))
  }
